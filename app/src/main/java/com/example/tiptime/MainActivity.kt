...
class MainActivity : AppCompatActivity() {
    ...
    private fun calculateTip() {
        ...
        // If the switch for rounding up the tip toggled on (isChecked is true), then round up the tip
    ...
    private fun displayTip(tip: Double) {
        // Create a number format instance without currency symbol
        val numberFormat = NumberFormat.getNumberInstance().apply {
            maximumFractionDigits = 2
            minimumFractionDigits = 2
        }
        val formattedTip = numberFormat.format(tip) + " AED"
